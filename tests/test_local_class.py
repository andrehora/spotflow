...
class TestLocalClassFlow(unittest.TestCase):
    ...
    def test_has_local_class_1(self):
        target_entity_name = 'tests.stub_sut.HasLocalClass.LocalClass1.local'
    ...
    def test_has_local_class_2(self):
        ...
        flows = result['tests.stub_sut.HasLocalClass.has_local_class'].flows
        self.assertEqual(len(flows), 1)
        self.assertEqual(flows[0].run_lines, [535, 539])

        flows = result['tests.stub_sut.HasLocalClass.has_local_class.<locals>.LocalClass2.local'].flows
        self.assertEqual(len(flows), 1)
    ...
    def test_has_local_class_3(self):
        ...
        flows = result['tests.stub_sut.has_local_class'].flows
        self.assertEqual(len(flows), 1)
        self.assertEqual(flows[0].run_lines, [544, 548])

        flows = result['tests.stub_sut.has_local_class.<locals>.LocalClass3.local'].flows
        self.assertEqual(len(flows), 1)
