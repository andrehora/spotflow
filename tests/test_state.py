...
class TestState(unittest.TestCase):
    ...
    def test_change_var_state(self):
        target_method_name = 'tests.stub_sut.ChangeState.change_var_state'
        func = TestChangeState().test_change_var_state

        result = run_and_flow_func(func, [target_method_name])

        self.assertEqual(result[target_method_name].calls[0].run_lines, [59, 60, 61])
        self.assertEqual(result[target_method_name].target_method_name, 'change_var_state')

        call_state = result[target_method_name].calls[0].call_state
    ...
    def test_change_arg_state(self):
        target_method_name = 'tests.stub_sut.ChangeState.change_arg_state'
        func = TestChangeState().test_change_arg_state

        result = run_and_flow_func(func, [target_method_name])

        self.assertEqual(result[target_method_name].calls[0].run_lines, [64, 65, 66])
        self.assertEqual(result[target_method_name].target_method_name, 'change_arg_state')
    ...
    def test_change_var_state_with_conditional_true(self):
        target_method_name = 'tests.stub_sut.ChangeState.change_var_state_with_conditional'
        func = TestChangeState().test_change_var_state_with_conditional_true

        result = run_and_flow_func(func, [target_method_name])

        self.assertEqual(result[target_method_name].calls[0].run_lines, [69, 70, 71])
        self.assertEqual(result[target_method_name].target_method_name, 'change_var_state_with_conditional')
    ...
    def test_change_var_state_with_conditional_false(self):
        ...
        self.assertEqual(result[target_method_name].calls[0].run_lines, [69, 70, 73])
        self.assertEqual(result[target_method_name].target_method_name, 'change_var_state_with_conditional')
    ...
    def test_change_multiple_vars_states(self):
        target_method_name = 'tests.stub_sut.ChangeState.change_multiple_vars_states'
        func = TestChangeState().test_change_multiple_vars_states

        result = run_and_flow_func(func, [target_method_name])

        self.assertEqual(result[target_method_name].calls[0].run_lines, [76, 77, 79, 80])
        self.assertEqual(result[target_method_name].target_method_name, 'change_multiple_vars_states')
    ...
    def test_change_list_state(self):
        target_method_name = 'tests.stub_sut.ChangeState.change_list_state'
        func = TestChangeState().test_change_list_state

        result = run_and_flow_func(func, [target_method_name])

        self.assertEqual(result[target_method_name].calls[0].run_lines, [83, 84, 85, 86, 87, 88, 89])
        self.assertEqual(result[target_method_name].target_method_name, 'change_list_state')
    ...
    def test_change_var_state_with_loop(self):
        target_method_name = 'tests.stub_sut.ChangeState.change_var_state_with_loop'
        func = TestChangeState().test_change_var_state_with_loop

        result = run_and_flow_func(func, [target_method_name])

        self.assertEqual(result[target_method_name].calls[0].run_lines, [92, 93, 94, 93, 94, 93, 94, 93, 94, 93])
        self.assertEqual(result[target_method_name].calls[0].distinct_run_lines(), [92, 93, 94])
        self.assertEqual(result[target_method_name].target_method_name, 'change_var_state_with_loop')
    ...
    def test_change_instance_var(self):
        target_method_name = 'tests.stub_sut.ChangeState.change_instance_var'
        func = TestChangeState().test_change_instance_var

        result = run_and_flow_func(func, [target_method_name])

        self.assertEqual(result[target_method_name].calls[0].run_lines, [100, 101])
        self.assertEqual(result[target_method_name].target_method_name, 'change_instance_var')
    ...
    def test_init_with_instance_var(self):
        target_method_name = 'tests.stub_sut.ChangeState.__init__'
        func = TestChangeState().test_init

        result = run_and_flow_func(func, [target_method_name])

        self.assertEqual(result[target_method_name].calls[0].run_lines, [97])
        self.assertEqual(result[target_method_name].target_method_name, '__init__')
    ...
    def test_keep_var_state(self):
        target_method_name = 'tests.stub_sut.ChangeState.keep_var_state'
        func = TestChangeState().test_keep_var_state

        result = run_and_flow_func(func, [target_method_name])

        self.assertEqual(result[target_method_name].calls[0].run_lines, [104, 105, 106])
        self.assertEqual(result[target_method_name].target_method_name, 'keep_var_state')
